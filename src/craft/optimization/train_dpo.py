"""
Unified Swift DPO training script for both R1 and Qwen models
"""
...
def get_model_config(model_type):
    ...
    configs = {
        "r1": {
            "model": "./custom/merged-quantized-<reference model>-bnb-nf4",
            "model_name": "R1Llama70BFineTuned"
        },
        "qwen": {
            "model": "./custom/merged-quantized-<reference model>-bnb-nf4",
            "model_name": "QwQ32BFineTuned"
        }
    }
...
def run_swift_training(model_type, dataset_path=None):
    """Run Swift DPO training using command line interface with multi-GPU for speed"""
    ...
    if dataset_path is None:
        dataset_path = f"data/dpo_data/{model_name.split('FineTuned')[0]}-dpo.jsonl"
    
    output_dir = f"outputs/{model_name.lower()}-dpo"
    
    print(f"=== DPO Training {model_name} Model ===")
    print(f"Model: {model}")
    ...
    # Build the swift dpo command with optimal settings
    cmd = [
        "swift", "dpo",
        "--model_type", "llama3",
        "--model_id", model,
    ...
    try:
        ...
        print(f"DPO training failed with error: {e}")
...
def main():
    parser = argparse.ArgumentParser(description='Unified Swift DPO Training Script')
    parser.add_argument('--model', type=str, choices=['r1', 'qwen'], required=True,
                       help='Model type to train (r1 or qwen)')
    parser.add_argument('--dataset', type=str, default=None,
                       help='Path to DPO dataset (optional, will use default if not provided)')
    parser.add_argument('--validate-only', action='store_true',
                       help='Only validate dataset without training')
    
    args = parser.parse_args()
    
    print(f"Starting DPO training for {args.model.upper()} model...")
    
    # Get model configuration
    config = get_model_config(args.model)
    dataset_path = args.dataset or f"data/dpo_data/{config['model_name'].split('FineTuned')[0]}-dpo.jsonl"
    ...
    if args.validate_only:
        ...
        if success:
            print(f"DPO training completed successfully for {args.model.upper()} model!")
        else:
            print(f"DPO training failed for {args.model.upper()} model!")
            exit(1)
